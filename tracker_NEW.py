...
class OBSWindow(tk.Toplevel):
    def __init__(self, parent, moon_rows, capture_row, ability_row,
        ...
        # --- Standard kingdom moon rows, with Cloud slotted between Wooded (index 3) and Lost (index 4) ---
        self.moon_obs_rows = []
        self.cloud_obs = None
        for i, row in enumerate(moon_rows):
            ...
            # Insert Cloud row after Wooded (index 3)
            if i == 3 and self.cloud_row and self.cloud_enabled:
                self.cloud_obs = OBSMoonRow(
                    self.moon_frame, self.cloud_row, self.bg_color,
                    white_icons_ref=self.white_icons_ref
                )
                self.cloud_obs.pack(pady=2, padx=6, anchor="w")
    ...
    def _update_moon_total(self):
        ...
        # Add cloud row only when cloud is enabled
        if self.cloud_enabled and self.cloud_row:
            total += self.cloud_row.count
    ...
    def refresh_special_rows(self, cap_enabled, star_enabled, dark_enabled, cloud_enabled=False):
        ...
        # Cloud row — between Wooded (obs index 3) and Lost (obs index 4)
        if cloud_enabled and self.cloud_obs is None and self.cloud_row:
            self.cloud_obs = OBSMoonRow(
                self.moon_frame, self.cloud_row, self.bg_color,
                white_icons_ref=self.white_icons_ref
            )
            # After Wooded obs row (index 3), before Lost obs row (index 4)
            if len(self.moon_obs_rows) > 4:
                self.cloud_obs.pack(pady=2, padx=6, anchor="w",
                                    before=self.moon_obs_rows[4])
            else:
                self.cloud_obs.pack(pady=2, padx=6, anchor="w")
        elif not cloud_enabled and self.cloud_obs:
            self.cloud_obs.destroy()
            self.cloud_obs = None
    ...
    def refresh_sidebar_rows(self, cap_visible, captures_visible, ability_visible):
        ...
        Order is always: Cap → Capture → Ability, regardless of toggle sequence."""
        ...
        # Always unpack all three first, then re-pack in fixed order
        for widget in (self.obs_sidebar_cap, self.obs_sidebar_star, self.obs_sidebar_ability):
            if widget:
                widget.pack_forget()

        if cap_visible and self.obs_sidebar_cap:
            self.obs_sidebar_cap.pack(pady=(4, 2), anchor="center")
        if captures_visible and self.obs_sidebar_star:
            self.obs_sidebar_star.pack(pady=(2, 2), anchor="center")
        if ability_visible and self.obs_sidebar_ability:
    ...
    def toggle_bg(self):
        ...
        if self.cloud_obs:
            self.cloud_obs.set_bg(self.bg_color)
        if self.star_obs:
...
class TrackerApp(tk.Tk):
    def __init__(self):
        ...
        # Cloud row: will be packed between Wooded and Lost (before moon_rows[4])
        self.cloud_row = MoonRow(self.left_column, resource_path("assets/Cloud.png"), app=self)
        ...
        controls_frame = tk.Frame(self.right_sidebar, bg=BG_COLOR)
        controls_frame.grid(row=2, column=0, pady=(20, 10))

        button_frame = tk.Frame(controls_frame, bg=BG_COLOR)
        button_frame.pack()

        # Rounded bottom buttons
        ctk.CTkButton(button_frame, text="Open OBS Overlay", command=self.open_obs,
                      font=FONT_NORMAL, corner_radius=12).pack(pady=5)
        ctk.CTkButton(button_frame, text="Toggle OBS BG", command=self.toggle_obs_bg,
                      font=FONT_NORMAL, corner_radius=12).pack()
        ctk.CTkButton(button_frame, text="Clear", command=self.reset_all_moons,
                      font=FONT_NORMAL, corner_radius=12).pack(pady=5)

        # Moon Cave captures — always visible, above Cave Skip
        self.left_captures = CaptureRow(controls_frame)
        self.left_captures.pack(pady=(0, 8))

        # Notes / Ability row (Cave Skip icons + Notes button) — always visible
        self.right_captures = AbilityRow(controls_frame, app=self)
        self.right_captures.pack(pady=(8, 0))

        # --- Row 1: Cap counter — toggled by Cap button ---
        self.sidebar_cap_row = SimpleCounterRow(controls_frame, resource_path("assets/Cap.png"), self)
        # hidden by default

        # --- Row 2: Captures counter (Spark_pylon) — toggled by Captures button ---
        self.sidebar_star_row = SimpleCounterRow(controls_frame, resource_path("assets/Spark_pylon_Capture.png"), self)
        # hidden by default

        # --- Row 3: Abilities counter — toggled by Ability button ---
    ...
    def _repack_special_rows(self):
        ...
        Enforce exact row order:
          Cap (if visible) → Cascade…Wooded → Cloud (if visible) → Lost…Bowser → Dark (if visible) → Star (if visible)
        ...
        wooded_row = self.moon_rows[3]      # Wooded  (index 3: Cascade, Sand, Lake, Wooded)
        lost_row = self.moon_rows[4]        # Lost
        ...
        # --- Cloud: between Wooded and Lost ---
        if self.cloud_enabled:
            self.cloud_row.pack_forget()
            self.cloud_row.pack(after=wooded_row, pady=5)
        else:
            self.cloud_row.pack_forget()
    ...
    def _repack_sidebar_rows(self):
        """Show/hide the three right-sidebar toggle rows in fixed order below Notes."""
        self.sidebar_cap_row.pack_forget()
        self.sidebar_star_row.pack_forget()
        self.sidebar_ability_row.pack_forget()
        if self.sidebar_cap_visible:
            self.sidebar_cap_row.pack(pady=(8, 0), anchor="center")
        if self.sidebar_captures_visible:
            self.sidebar_star_row.pack(pady=(4, 0), anchor="center")
        if self.sidebar_ability_visible:
            self.sidebar_ability_row.pack(pady=(4, 0), anchor="center")
    ...
    def save_state(self):
        ...
        data = {
            ...
            "cloud_max": self.cloud_row.max_val,
    ...
    def load_state(self):
        ...
        try:
            ...
            cloud_max = data.get("cloud_max", None)
            self.cloud_row.max_val = cloud_max
            if cloud_max is not None:
                self.cloud_row.max_var.set(str(cloud_max))
            self.cloud_row.update_label()
